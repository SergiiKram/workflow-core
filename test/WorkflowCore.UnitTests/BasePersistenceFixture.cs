...
namespace WorkflowCore.UnitTests
{
    public abstract class BasePersistenceFixture
    {
        ...
        [Fact]
        public async Task CreateNewWorkflow_should_create_duplicates_without_reference()
        {
            var workflow = new WorkflowInstance
            {
                Data = new { Value1 = 7 },
            ...
            var workflowId1 = await Subject.CreateNewWorkflow(workflow);

            workflow.Id = null;

            var workflowId2 = await Subject.CreateNewWorkflow(workflow);
        ...
        [Fact]
        public async Task CreateNewWorkflow_with_duplicate_reference_should_fail()
        {
            var workflow = new WorkflowInstance
            {
                Data = new { Value1 = 7 },
            ...
            if (this.GetType().Name == "RedisPersistenceProviderFixture")
            {
        ...
        [Fact]
        public async Task ConcurrentPersistWorkflow()
        {
                ...
                actions.Add(async () =>
            {
                    ...
                    await subject.PersistWorkflow(newWorkflow); // It will throw an exception if the persistence provider occurred resource competition.
                });
            }
        ...
        public async Task CreateNewEvent()
        {
            ...
            var eventId = await Subject.CreateEvent(@event);
